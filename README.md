...
## 이것이 무엇을 위한 프로젝트인가?
실험실에서 배양 접시 이미지를 다룰 때 자주 발생하는 문제는 다음과 같습니다.

- 이미지마다 콜로니를 **매번 수동으로 다시 찾고 라벨링**해야 함
- 같은 샘플의 시계열/유사 촬영 이미지인데도, 콜로니 위치가 조금씩 달라져 **이전 라벨을 그대로 재사용하기 어려움**
- 결과를 분석용 CSV로 넘기기 전에, 사람이 마지막으로 정리해야 하는 시간이 큼
...
- 변환된 이전 점 \\(\hat{q}_j=Tq_j\\)와 현재 점 \\(p_i\\) 사이 거리를 계산하여 비용행렬 \\(C\\)를 만듭니다.
